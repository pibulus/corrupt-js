...
(function (root, factory) {
  ...
  const styles = {
    witch: {
      chars: '†‡§¶•◦∘⊙',
      safeChars: '#@%&*!?',
      rate: 0.12,  // More subtle
      maxCorruptions: 1,  // Just one char at a time
      animation: 'shake',
      duration: 800  // Slower, more controlled
    },
    matrix: {
      chars: '01|/\\-_[]{}()=+',
    ...
    dots: {
      ...
      rate: 0.25,
      maxCorruptions: 3,
      animation: 'twinkle',  // Better animation for dots
      duration: 800
    },
    melt: {
      chars: '▼▾▿╱╲╳¦┊┋│',
  ...
  class Corrupt {
    ...
    corrupt(element, styleName, rate, max) {
      ...
      switch(style.animation) {
        ...
        case 'wave':
          ...
          break;
        case 'cutup':
          this.cutupCorrupt(element, text, activeStyle, useRate, useMax);
          break;
        case 'collapse':
          this.collapseCorrupt(element, text, activeStyle, useRate, useMax);
          break;
        default:
    ...
    // Burroughs cut-up effect - rearranges words and corrupts characters
    cutupCorrupt(element, text, style, rate, max) {
      ...
      if (wordIndices.length < 2) {
        // Not enough words to rearrange, fall back to character corruption
        element.textContent = this.corruptText(text, '†‡§¶•◦∘⊙|\\/[]{}()✦✧⋆∗◉◎', rate, max);
        return;
      }
      ...
      const shuffle = () => {
        ...
        const numSwaps = Math.min(max, Math.ceil(wordIndices.length * rate));

        for (let s = 0; s < numSwaps; s++) {
          const a = wordIndices[Math.floor(Math.random() * wordIndices.length)];
          const b = wordIndices[Math.floor(Math.random() * wordIndices.length)];
          [display[a], display[b]] = [display[b], display[a]];
        }

        // Subtle character corruption on rearranged words (increases over time)
        reshuffleCount++;
        const charCorruptChance = Math.min(0.04 + reshuffleCount * 0.01, 0.15);

        for (let i = 0; i < display.length; i++) {
          if (display[i].trim() && Math.random() < charCorruptChance) {
            const word = display[i];
            const pos = Math.floor(Math.random() * word.length);
            display[i] = word.substring(0, pos) +
      ...
      shuffle();
      const interval = setInterval(shuffle, 250);
      this.activeAnimations.set(element, interval);
    }

    // Collapse effect - text progressively degrades through three phases:
    // Phase 1 (glitch): occult/tech character flicker
    // Phase 2 (decay): block elements and screen static
    // Phase 3 (sigil): abstract geometric glyphs, full collapse
    collapseCorrupt(element, text, style, rate, max) {
      const phaseChars = [
        '†‡§¶•◦∘⊙╱╲╳⌁⌂⌃⌇⌑',
        '▓▒░█▄▀■□▪▫◊○●◐◑◒◓▁▃▅▇',
        '◈◇◆◉◎⦿◍⬡⬢⬣⟡⟢⧫⧬⧭⧮⧯⦰⦱⦲',
      ...
      const textArray = text.split('');
      let startTime = null;
      const totalDuration = style.duration || 2500;
      let sigilInterval = null;
      const self = this;

      const getChars = function(phase) {
        return self.options.safeMode ? safePhaseChars[phase] : phaseChars[phase];
      };

      const collapse = function(timestamp) {
        if (!startTime) startTime = timestamp;
        const progress = Math.min((timestamp - startTime) / totalDuration, 1.0);
        const display = [...textArray];

        var phase, corruptChance;

        if (progress < 0.33) {
          phase = 0;
          corruptChance = rate * (progress / 0.33);
        } else if (progress < 0.66) {
          phase = 1;
          corruptChance = rate + (0.9 - rate) * ((progress - 0.33) / 0.33);
        } else {
          phase = 2;
          corruptChance = 0.9 + 0.1 * Math.min((progress - 0.66) / 0.34, 1);
        }

        var chars = getChars(phase);

        for (var i = 0; i < display.length; i++) {
          if (self.options.preserveSpaces && textArray[i] === ' ') continue;
          if (Math.random() < corruptChance) {
            display[i] = chars[Math.floor(Math.random() * chars.length)];
          }
        }

        // Subtle screen shake during decay/sigil phases
        if (phase >= 1 && progress < 1) {
          element.style.display = 'inline-block';
          var intensity = (progress - 0.33) * 3;
          var sx = (Math.random() - 0.5) * intensity;
          var sy = (Math.random() - 0.5) * intensity * 0.5;
        ...
        if (progress < 1) {
          self.activeAnimations.set(element, requestAnimationFrame(collapse));
        } else {
          ...
          sigilInterval = setInterval(function() {
            ...
            for (var j = 0; j < text.length; j++) {
              if (self.options.preserveSpaces && textArray[j] === ' ') {
                sigil.push(' ');
              } else {
                sigil.push(sc[Math.floor(Math.random() * sc.length)]);
              }
            }
            element.textContent = sigil.join('');
          }, 150);
          self.activeAnimations.set(element, sigilInterval);
        }
      };
    ...
    dripCorrupt(element, text, style, rate, max) {
      const chars = text.split('');
      let dripping = new Set();
      let frame = 0;

      const drip = () => {
        // Reset every 20 frames
        if (frame % 20 === 0) {
          dripping.clear();
        }
      ...
      // Run continuously
      drip();
      const interval = setInterval(drip, 50);
      this.activeAnimations.set(element, interval);
    }
    ...
    twinkleCorrupt(element, text, style, rate, max) {
      ...
      // Run continuously with setInterval
      twinkle(); // Initial run
      const interval = setInterval(twinkle, style.duration / 10 || 100);
      this.activeAnimations.set(element, interval);
    }
    ...
    static corruptString(text, style = 'witch', rate = 0.3, max = 2) {
      // Burroughs style uses word rearrangement instead of char replacement
      if (style === 'burroughs') {
        return Corrupt.cutupString(text, rate);
      }

      // Collapse style returns fully sigil-collapsed text
      if (style === 'collapse') {
        const sigilChars = '◈◇◆◉◎⦿◍⬡⬢⬣⟡⟢⧫⧬⧭⧮⧯⦰⦱⦲';
        return text.split('').map((char, i) => {
          if (i === 0 || i === text.length - 1) return char;
          if (char === ' ') return char;
          return sigilChars[Math.floor(Math.random() * sigilChars.length)];
        }).join('');
      }
    ...
    // Utility method for Burroughs cut-up style one-off
    static cutupString(text, intensity = 0.35) {
      const tokens = text.split(/(\s+)/);
      const wordIndices = [];
      ...
      if (wordIndices.length < 2) return text;

      const numSwaps = Math.ceil(wordIndices.length * intensity);
