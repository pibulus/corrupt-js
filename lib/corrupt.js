...
(function (root, factory) {
  ...
  const styles = {
    witchhouse: {
      chars: '†‡§¶∞░▒▓█▀▄#%&',
      safeChars: safeChars.witchhouse,
      rate: 0.3,
      maxCorruptions: 2,
      defaultMode: 'shake',
  ...
  class Corrupt {
    ...
    init() {
      this.elements.forEach(el => {
        ...
        const elementMode = el.dataset.corruptMode || null;
        const elementEasing = el.dataset.corruptEasing || null;
    ...
    corruptWithOptions(element) {
      ...
      const style = styles[opts.style] || styles.glitch;

      // Use element-specific or style-specific defaults
      const mode = opts.mode || style.defaultMode || this.options.mode;
      const easing = opts.easing || style.defaultEasing || this.options.easing;
      const duration = parseInt(opts.duration) || style.defaultDuration || this.options.duration;

      // Temporarily override instance options for this element
      const originalMode = this.options.mode;
      const originalEasing = this.options.easing;
      ...
      this.options.easing = originalEasing;
    ...
    corrupt(element, styleName, rate, max) {
      ...
      switch(mode) {
        ...
        case 'flicker':
          ...
          break;
        case 'drip':
          this.dripCorrupt(element, text, style, useRate, useMax);
          break;
        case 'twinkle':
          this.twinkleCorrupt(element, text, style, useRate, useMax);
          break;
        default:
